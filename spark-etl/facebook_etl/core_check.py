...
fetch_new_jobs_sql = """
        select id, report_id from facebook_staging.job_manager where report_status = 'Job Posted' or report_status = 'Job Running'
        """
...
def core_check():
    ...
    for job in fetch_posted_jobs():
        ...
        if stat == "Job Completed":
            ...
            jobs_failed += 1
        jobs_checked += 1
    print(f'Job Check Completed\n ---Jobs checked:{jobs_checked}, Success: {jobs_success}, Jobs Failed: {jobs_failed}')
