...
-- humana pharmacy proc
create or replace procedure facebook_prod.humana_pharm_standard()
language plpgsql
as $$ begin
drop table facebook_prod.humana_pharm_standard;

create table facebook_prod.humana_pharm_standard as
...
from (
        select 
            ...
            ,fds.video_completes
            from facebook.fact_daily_standard as fds
        where account_id = 303659597906507) as fds
left join (
        select 
            ad_id
            ,adgroup_id
        ...
        ,json_object_agg(action_name, count_value) as count_value
        from facebook.fact_daily_actions
        where account_id = 303659597906507
        group by 1,2,3,4,5
        order by date) as fda on (fda.account_id, fda.ad_id, fda.date) = (fds.account_id, fds.ad_id, fds.date)
left join (
        select 
            ...
            ,optimization_goal
        from facebook.campaigns
        where account_id = 303659597906507) as cmp on (cmp.account_id, cmp.id) = (fds.account_id, fds.campaign_id)
left join (
        select 
            name
            ,id
            ...
            ,attribution_setting
        from facebook.adgroups
        where account_id = 303659597906507) as ag on (ag.account_id, ag.id) = (fds.account_id, fds.adgroup_id)
left join (
        select
            name
            ,id
            ,account_id
        from facebook.ads
        where account_id = 303659597906507) as ad on (ad.account_id, ad.id) = (fds.account_id, fds.ad_id);
...
-- humana pharmacy proc
create or replace procedure facebook_prod.xumo_daily_standard()
language plpgsql
as $$ begin
drop table facebook_prod.xumo_daily_standard;

create table facebook_prod.xumo_daily_standard as
...
from (
        select 
            ...
            ,fds.video_completes
            from facebook.fact_daily_standard as fds
        where account_id = 577292516735129) as fds
left join (
        select 
            ad_id
            ,adgroup_id
        ...
        ,json_object_agg(action_name, count_value) as count_value
        from facebook.fact_daily_actions
        where account_id = 577292516735129
        group by 1,2,3,4,5
        order by date) as fda on (fda.account_id, fda.ad_id, fda.date) = (fds.account_id, fds.ad_id, fds.date)
left join (
        select 
            ...
            ,optimization_goal
        from facebook.campaigns
        where account_id = 577292516735129) as cmp on (cmp.account_id, cmp.id) = (fds.account_id, fds.campaign_id)
left join (
        select 
            name
            ,id
            ...
            ,attribution_setting
        from facebook.adgroups
        where account_id = 577292516735129) as ag on (ag.account_id, ag.id) = (fds.account_id, fds.adgroup_id)
left join (
        select
            name
            ,id
            ,account_id
        from facebook.ads
        where account_id = 577292516735129) as ad on (ad.account_id, ad.id) = (fds.account_id, fds.ad_id);
